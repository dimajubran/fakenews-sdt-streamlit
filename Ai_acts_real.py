...
def compute_outcomes(params: Dict[str, float]) -> Dict[str, float]:
    """
    Architecture 3: AI-Acts-on-Real.
    AI autonomously decides Np->Nh. Human decides Sp and Mp.
    """
    ...
    # 2) Human handles Sp and Mp only (AI auto-handles Np).
    ratio = payoff_ratio(params)

    posterior_Sp = posterior_signal_given_cue(Ps, ai["pSp_S"], ai["pSp_N"])
    posterior_Mp = posterior_signal_given_cue(Ps, ai["pMp_S"], ai["pMp_N"])

    # Cue-conditioned normative thresholds for the human branches.
    Bstar_Sp = bstar_for_posterior(posterior_Sp, dH, ratio)
    Bstar_Mp = bstar_for_posterior(posterior_Mp, dH, ratio)

    pSh_S_Sp, pSh_N_Sp = human_sh_probabilities(dH, Bstar_Sp)
    pSh_S_Mp, pSh_N_Mp = human_sh_probabilities(dH, Bstar_Mp)

    # 3) Routing:
    #    Np -> AI final Nh, Sp/Mp -> human final Sh/Nh.
    TP = Ps * (ai["pSp_S"] * pSh_S_Sp + ai["pMp_S"] * pSh_S_Mp)
    FN = Ps * (
        ai["pNp_S"]
        + ai["pSp_S"] * (1.0 - pSh_S_Sp)
        + ai["pMp_S"] * (1.0 - pSh_S_Mp)
    )
    FP = Pn * (ai["pSp_N"] * pSh_N_Sp + ai["pMp_N"] * pSh_N_Mp)
    TN = Pn * (
        ai["pNp_N"]
        + ai["pSp_N"] * (1.0 - pSh_N_Sp)
        + ai["pMp_N"] * (1.0 - pSh_N_Mp)
    )
    ...
    score_explicit = (
        ...
        * (
            ai["pNp_S"] * VFN
            + ai["pSp_S"] * (pSh_S_Sp * VTP + (1.0 - pSh_S_Sp) * VFN)
            + ai["pMp_S"] * (pSh_S_Mp * VTP + (1.0 - pSh_S_Mp) * VFN)
        )
        + Pn
        * (
            ai["pNp_N"] * VTN
            + ai["pSp_N"] * (pSh_N_Sp * VFP + (1.0 - pSh_N_Sp) * VTN)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run AI-Acts-on-Real simulation sweeps.")
